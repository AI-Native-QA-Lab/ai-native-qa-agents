...
| v0.3 | 测试生成、执行、修复与重试 |
| v0.4 | 测试有效性与变异证据循环 |
| v0.5–v1.0 | 见 [路线图](ROADMAP_AND_VERSION_DESIGN.md) |

当前包装版本为 **0.4.0**，可执行基线覆盖 v0.1 审查、v0.2 需求智能、v0.3 测试工程与 v0.4 测试有效性和变异证据。
...
v0.4 循环具备有界执行和证据驱动特征。变异只是证据提供方；未经评估的真实项目、模型、包、部署与索引结论仍明确不属于本次发布证据。
...
本仓库已提供至 v0.4.0 的可执行基线，是一个参考架构；v0.4.0 不对未经评估的外部系统作出生产就绪声明。
