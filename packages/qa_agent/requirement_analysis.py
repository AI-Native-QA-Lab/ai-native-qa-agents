...
from .requirements import AcceptanceCriterion, Requirement, RequirementContext, RequirementResult, RiskItem, TestabilityFinding
...
class RequirementAnalysisService:
    def analyze(self, request: RequirementRequest) -> RequirementResult:
        ...
        if request.max_actions < 4:
            ...
            return result
        criterion_budget = request.max_actions - 3
        for iteration, criterion in enumerate(requirement.acceptance_criteria[:criterion_budget], 1):
        ...
        if len(requirement.acceptance_criteria) > criterion_budget:
            result.termination_reason = "BUDGET_EXHAUSTED"
            result.loop_trace = [LoopTrace(1, "parse-requirement", "completed"), LoopTrace(2, "analyze-testability", "budget-exhausted")]
            return result
        actions: dict[str, dict[bool, tuple[AcceptanceCriterion, Evidence]]] = {}
        for criterion, evidence in zip(requirement.acceptance_criteria, result.evidence):
            ...
            opposite = actions.get(action, {}).get(not negated)
            if opposite is not None:
                    evidence_ids = (opposite[1].id, evidence.id)
                    ...
                    result.risks.append(RiskItem(f"R-{len(result.risks)+1:03d}", "conflicting_criteria", "high", "Conflicting acceptance criteria prevent a deterministic test outcome", (opposite[0].id, criterion.id), evidence_ids))
            actions.setdefault(action, {})[negated] = (criterion, evidence)
        ...
        result.context = RequirementContext(f"CTX-{requirement.id}", requirement.id, tuple(item.id for item in result.evidence))
